...
- **Allow remote computer use** — the remote watcher can click (left/right/middle), drag, scroll, type, use modifier shortcuts (⌘/Ctrl+C), paste clipboard text, and **drop files** onto the session. Copy on the host is pushed to the watcher (text and PNG images up to 32 MB, never truncated; large PNGs go in 24 KB chunks). Watchers can paste PNG or other images (JPEG/WebP are converted to PNG). Files land in the origin `inbox/` folder (next to `config.json`, up to 2 GB, 24 KB chunks streamed from `File.slice` and appended to disk, resumes from `.part` after a drop). Get/download streams the file from disk instead of buffering it in RAM. On the watch page, Get uses `showSaveFilePicker` so chunks write to disk instead of sitting in browser memory (Blob download is the fallback). Kill switch: uncheck it.
