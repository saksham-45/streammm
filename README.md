...
| fps | 30 |
| Audio | Off by default. Settings **Capture audio** muxes AAC into the live fMP4 (H.264/HEVC, macOS) and **reveals an audio device picker** (default mic, or BlackHole/Loopback for system audio). Watchers get an **Unmute** control (autoplay starts muted). Settings **Allow watcher to talk** reveals a **Talk** button; their mic plays continuously on the host speakers (ffmpeg/ffplay s16le pipe, `afplay` fallback). Talk requests browser echo cancellation; if Capture audio is also on, Talk is **half-duplex** (the live stream mutes while you speak, and the host input is ducked so the mic does not pick up the speakers) and Settings reveals that hint. |
...
**Allow unattended access** (Settings) reveals a persistent password. After the PIN expires, watchers join with that password on the same unlock field (`POST /api/otp/redeem` `{ "pin": "…" }`). The origin stores only a SHA-256 hash; GET `/api/config` reports `access.password_set` and never the hash. Uncheck to disable. Unattended access, keep-awake, or remote control also reveals **Wake-on-LAN**: this Mac's MAC address, a **Copy MAC** button, and a **Send wake packet** button (`POST /api/wol` `{ "mac": "aa:bb:…" }`) that broadcasts a magic packet on the LAN. A sleeping origin cannot send that packet itself — copy the MAC and send it from another device on the network. The watch page also gets a **Copy MAC** control from host flags (persisted in the Worker) so you still have the address when the publisher is down. When the origin is not publishing, the watch page shows **host offline** (the viewer socket stays up on the Worker) and reveals a Wake-on-LAN hint with that Copy MAC. A sleeping origin cannot send the packet itself — the browser and Worker cannot UDP-broadcast. Firmware Wake-on-LAN must be on; a closed MacBook lid often will not wake.
...
- **Allow remote computer use** — the remote watcher can click (left/right/middle), drag, scroll, type, use modifier shortcuts (⌘/Ctrl+C), paste clipboard text, paste files, and **drop files** onto the session. Enabling it reveals a **Send-keys** bar for shortcuts the browser would steal (⌘Tab, ⌘W, ⌘Q, Alt+Tab, Alt+F4, Ctrl+Alt+Del, Lock) and a **session chat** panel so host and watcher can talk without those keystrokes hitting the remote desktop. The AI injector uses the same key actions. Copy on the host is pushed to the watcher (text, PNG images up to 128 MB, and Finder file copies into the inbox with a Save control on the watch page — never truncated; large PNGs go in 24 KB chunks, and a too-large image is rejected rather than truncated). Watchers can paste PNG or other images (JPEG/WebP are converted to PNG) and paste non-image files into the host inbox. Files land in the origin `inbox/` folder (next to `config.json`, up to 2 GB, 24 KB chunks streamed from `File.slice` and appended to disk, resumes from `.part` after a drop), are copied onto the host **Desktop**, **and** are placed on the host pasteboard as Finder files so Cmd-V pastes them into another folder. Get/download streams the file from disk instead of buffering it in RAM. Folders download as a STORE zip (`{name}.zip`) with the same 2 GB / 2000-entry cap; Get is revealed on folders as well as files. Checkboxes select multiple items and reveal bulk **Copy / Cut / Get / Delete** (Get of several items is `files.zip`; Copy/Cut queue all of them for Paste here). **Select all** (⌘/Ctrl+A) and Shift-click select a range from the last anchor; ⌘/Ctrl-click toggles. Drag onto a folder row, **Up**, or an Inbox/Home/Desktop/Documents/Downloads tab to **move** (⌥/Ctrl copies); dropping OS files onto a folder writes into that folder. **Split** reveals a second pane (defaults to Desktop) so you can keep two host folders open and drag between them. On the watch page, Get uses `showSaveFilePicker` so chunks write to disk; Safari/Firefox fall back to `GET /api/files/download`, which the Worker streams from the origin. Enabling remote control also reveals **Inbox / Home / Desktop / Documents / Downloads** browse (and **Up** into subfolders), **New folder**, **Rename**, **Copy / Cut** plus **Paste here** (move or copy between those folders), **Get** (files, folders as zip, or a multi-select zip), and **Delete** (files and folders; folder delete is recursive after confirm, capped at 2000 entries) (`file` / `list|get|delete` with `root` + `path`, or `GET`/`DELETE /api/files?root=desktop&path=…&name=…` on the origin; repeat `name=` or send `names` for a selection). **Drop / browse-upload** writes into the folder you are viewing (Inbox still also copies onto Desktop and the pasteboard). Kill switch: uncheck it.
- **Allow AI computer use** — the same watcher can submit a task in **Have AI use this computer**. The origin loops TYPE_SNAP JPEG → vision model JSON actions → the same injector (click/right-click/drag/type/key/paste/clipboard PNG/inbox file). File actions can also **list / mkdir / rename / copy / move / delete** host folders (same roots as the Files panel); a list result is appended as `[files]=JSON` on the next step. **Cancel AI** on the origin UI (`POST /api/computer-use/cancel`) stops a running loop.
...
One remote controller at a time. The origin UI shows a **REMOTE SESSION** banner with **End** (`POST /api/control/release`) while someone is driving. Optional **Block local input during remote session** swallows this Mac's HID until **⌘⇧Esc** or End (End cannot be clicked while locked). Optional **Blank the host screen during remote session** drives the physical displays to black (gamma table plus display brightness) so people nearby cannot see the desktop, including HDR / True Tone panels; capture still streams the real framebuffer. ⌘⇧Esc unblanks and reclaims. Optional **Keep this Mac awake for remote access** (revealed with unattended access or remote control) holds idle-sleep and display-sleep assertions so the origin stays reachable; a live remote session always holds them. Lid close still sleeps; on wake, capture and the publisher restart immediately. Optional **Lock this Mac when the remote session ends** injects Control+Command+Q when the watcher disconnects or you press End — not when you reclaim with ⌘⇧Esc. Optional **Record remote sessions** writes the live fMP4 to `recordings/` while someone is driving and reveals a Get list on the host UI. Watchers (and the host live view) also have a **Record** button that saves this view locally via MediaRecorder (WebM/MP4), independent of the host `recordings/` folder, and a **Full** button (F11) that fullscreen the live view. Escape exits fullscreen without injecting into the remote session. Local host input / the kill switch always wins when unlocked.
...
GET/POST /api/files     folder list (`?root=desktop&path=…`) / `{ "name", "data" }` (base64; 403 unless control on)
POST /api/files/mkdir   `{ "name", "root", "path" }` create a folder in the browsed directory
POST /api/files/rename  `{ "name", "to", "root", "path" }` rename a file or empty-safe folder
POST /api/files/copy    `{ "name", "root", "path", "toRoot", "toPath" }` copy into another browsed folder
POST /api/files/move    same shape; rename across folders (copy+delete if needed)
DELETE /api/files?name=…  remove a file (`root`/`path` optional; same auth as list)
GET  /api/files/download?name=…  (`root`/`path` optional; folders stream as `{name}.zip`)
GET  /api/recordings    host: session fMP4 list   GET /api/recordings/download?name=…
GET  /api/events      SSE
