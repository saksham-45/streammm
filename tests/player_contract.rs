...
fn prefers_websocket_typed_frames_not_2s5_seek() {
    ...
    assert!(
        js.contains("function deleteInboxFile")
            && js.contains("action: \"delete\"")
            && js.contains("textContent = \"Delete\""),
    ...
    assert!(
        js.contains("everything inside") && js.contains("deleteInboxFile(f.name, true)"),
        "folder Delete must confirm before removing a non-empty tree"
    );
    assert!(
    ...
    assert!(
        html.contains("id=\"file-sel-copy\"")
            && html.contains("id=\"file-sel-get\"")
            && js.contains("type = \"checkbox\"")
            && js.contains("names: names"),
...
fn worker_player_has_pin_unlock_and_ai_box() {
    ...
    assert!(
        s.contains("function deleteInboxFile")
    ...
    assert!(
        s.contains("everything inside") && s.contains("deleteInboxFile(f.name, true)"),
        "watch page folder Delete must confirm before removing a non-empty tree"
    );
    assert!(
    ...
    assert!(
        s.contains("id=\"file-sel-copy\"")
            && s.contains("type = \"checkbox\"")
            && s.contains("files.zip")
            && s.contains("names: names"),
